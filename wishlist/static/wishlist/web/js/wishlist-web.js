...
wishlistService.getList(page + 1).then((wishlists) => {
...
//카테고리별 위시리스트 보여주기
const categoryshowList = (category_wishlists) => {
    let category_text = ``;
    category_wishlists.forEach((wishlist) => {
        if(wishlists.length === 0) {
            category_text = `
                <!-- 카테고리에 게시글이 없을 때 출력 -->
                <div class="post-error">
...
wishlistService.getList(page, showList).then((text) => {
    div.innerHTML = text;
});

wishlistService.category(page, category, categoryshowList).then((category_text) => {
    div.innerHTML = category_text;
});
