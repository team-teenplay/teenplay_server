                    ...
                    <div class="play-writer-wrap">
                        ...
                        <div class="play-like-info-wrap">
                            <div class="play-like-wrap">
                                <button class="play-like-btn" value="${teenplayList['id']}">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="play-like-icon empty" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path>
                                    </svg>
                                    <svg xmlns="http://www.w3.org/2000/svg" class="play-like-icon full" viewBox="0 0 20 20" fill="currentColor">
                                        <path  fill-rule="evenodd" d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z" clip-rule="evenodd"></path>
                                    </svg>
                                </button>
                            </div>
                            <div class="play-like-count">${teenplayList['teenplay_like']}</div>
                        </div>
                    </div>
                    <!-- 제목(내용) -->
...
slideWrap.addEventListener("wheel", async (e) => {
    ...
    clubId=1
    page=3
    const showData = (teenplayList) => {
        return teenplayList
    }

    teenplayClubService.getList(clubId, page, (teenplayList) => {
        const processedData = showData(teenplayList);
        condition = 'id' in processedData


        if (e.deltaY > 0) {
            // 화면에서 인덱스 값을 클럽 id 값을 받아와서 해당 id 값의 teenplay가 더 있으면 값+ 해줘야하고 만약 가져온 data의 length 가 0 이면 멈춰야함
            // 페이지 번호를 그냥 가져온 다음에 그걸 기준으로 + 해준다

            if(condition && idx === videoWraps.length-1) {
                teenplayClubService.getList(clubId, page, showList).then((text) => {
            ...
            if(page != 1) {
                ...
                teenplayClubService.getList(clubId, page, showList).then((text)=> {
                    ...
                    requestAnimationFrame(() => {
                        setTimeout(()=> {
                            // 현재 재생 중인 요소(또는 새로운 요소)로 스크롤 조정
                            const newCurrentPlayingTag = document.querySelector('.play-each');
                            newCurrentPlayingTag.scrollIntoView({ behavior: 'smooth'});
                        },100)
                    });
