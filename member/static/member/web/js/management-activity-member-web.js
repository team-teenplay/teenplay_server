...
const statusAddEvent = () => {
    let memberSecessionBtns = document.querySelectorAll(".member-secession-btn");

    memberSecessionBtns.forEach((memberSecessionBtn) => {
        memberSecessionBtn.addEventListener("click", (e) => {
            ...
            kickOutModalContainer.querySelector(".kick-out-modal-title").innerText = `${targetName}님의 활동 참여를 취소하시겠습니까?`;
    ...
    kickOutModalContainerBtn.addEventListener("click", (e) => {
        ...
        if (e.target.className == "kick-out-btn") {
            ...
            setTimeout(() => {
                ...
                kickOutCheckModalContainer.querySelector(".modal-header-title").innerText = `${targetName}님의 활동 참여를 취소했습니다.`;
...
joinModalContainerBtns.forEach((joinModalContainerBtn) => {
    joinModalContainerBtn.addEventListener("click", (e) => {
        ...
        if (e.target.className == "join-btn") {
            // 데이터가 없어 임시 방편으로 사용
            target.querySelector(".member-status").innerHTML = `<div class="member-status-join-btn">참가중</div>`;
            stopAddEvent();

            setTimeout(() => {
                joinModalContainer.style.display = "none";
                joinCheckModalContainer.querySelector(".modal-header-title").innerText = `${targetName}님의 참가를 승인했습니다.`;
...
const stopAddEvent = () => {
    let memberStatusStopBtns = document.querySelectorAll(".member-status-join-btn");
    memberStatusStopBtns.forEach((memberStatusStopBtn) => {
        memberStatusStopBtn.addEventListener("click", (e) => {
            console.log("in");
            target = e.target.closest(".member-info-list");
            targetName = target.querySelector(".member-name").innerText;

            stopModalContainer.style.animation = "popUp 0.5s";
            stopModalContainer.querySelector(".stop-modal-title").innerText = `${targetName}님의 상태를 변경하시겠습니까?`;
...
stopModalContainerBtns.forEach((stopModalContainerBtn) => {
    stopModalContainerBtn.addEventListener("click", (e) => {
        stopModalContainer.style.animation = "popDown 0.5s";
        if (e.target.className == "stop-btn") {
            // 데이터가 없어 임시 방편으로 사용
            target.querySelector(".member-status").innerHTML = `<div class="member-status-stand-btn">참가대기</div>`;
            joinAddEvent();

            setTimeout(() => {
                stopModalContainer.style.display = "none";
                stopCheckModalContainer.querySelector(".modal-header-title").innerText = `${targetName}님의 상태를 변경했습니다.`;
                stopCheckModalContainer.style.animation = "popUp 0.5s";
                stopCheckModalContainer.style.display = "flex";
            }, 450);
        } else if (e.target.className == "stop-modal-cancle-btn") {
            setTimeout(() => {
                stopModalWrap.style.display = "none";
            }, 450);
        } else {
            stopCheckModalContainer.style.animation = "popDown 0.5s";
            setTimeout(() => {
                stopCheckModalContainer.style.removeProperty("animation");
                stopCheckModalContainer.style.display = "none";
                stopModalContainer.style.display = "flex";
                stopModalWrap.style.display = "none";
            }, 450);
        }
    });
